    ...
    <v-card class="pa-6 mx-auto" elevation="4" max-width="1100">
        ...
        <v-form ref="form" v-model="isValid" @submit.prevent="handleSubmit">
            <v-row dense>
                ...
                <v-col cols="12" md="6">
                    <v-text-field type="date" label="Rental Start Date" v-model="form.Rental_Start_Date" outlined
                        dense />
                </v-col>
                ...
                <v-col cols="12" md="4">
                    <v-text-field type="number" label="Bedrooms Count" v-model.number="form.Bedrooms_Count" outlined
                        dense min="1" />
                </v-col>
                ...
                <v-col cols="12" md="4">
                    <v-checkbox label="Free Rental" v-model="form.Free_Rental" />
                </v-col>
                ...
                <v-col cols="12" md="6">
                    <v-select label="Send Billing *" :items="['Yes', 'No', 'Select']" v-model="sendBillingLocal" outlined dense
                        :rules="[v => !!v || 'Send Billing is required']" required />
                </v-col>
...
export default {
    ...
    computed: {
        ...
        discountLabel() {
            const pct = Math.round((1 - this.discountMultiplier) * 100);
            return `${pct}%`;
        },
    ...
    methods: {
        handleSubmit() {
            ...
            this.$emit("submit", {
                ...
                // ================= Box Logic =================
                Bedrooms_Count: this.form.Bedrooms_Count,
                Total_Box_Count: this.totalBoxCount,

                // ================= Options =================
                Free_Rental: this.form.Free_Rental,
                Include_Bubble_Wrap: this.form.Include_Bubble_Wrap,

                // ================= Pricing =================
                Box_Tier: this.boxTier,
                Weeks_Count: this.weeks,
                Base_Price: this.basePrice,
                Additional_Week_Rate: this.additionalWeekRate,
                Discount_Percentage: Math.round((1 - this.discountMultiplier) * 100),
                Bubble_Wrap_Price: this.form.Include_Bubble_Wrap ? 60 : 0,
